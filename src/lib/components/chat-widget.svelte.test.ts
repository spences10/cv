...
describe('ChatWidget', () => {
	it('should render expanded by default with header', async () => {
		...
		const title = page.getByText(/Got questions about my experience/);
		await expect.element(title).toBeInTheDocument();
	});

	it('should show logistics questions initially', async () => {
		render(ChatWidget);

		const q1 = page.getByRole('button', {
			name: 'Available now? Notice period?',
		});
		const q2 = page.getByRole('button', {
			name: 'Contract or permanent?',
		});
		const q3 = page.getByRole('button', {
			name: 'Remote only or open to hybrid?',
		});
	...
	it('should show Quick questions label initially', async () => {
		render(ChatWidget);

		const label = page.getByText('Quick questions');
